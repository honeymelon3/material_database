	...
	var sql1='select param_scope from alloy_param where id=\''+req.params.param_id+'\';';
	 ...
	 my_conn.query(sql1, function (result) {
		//  res.jsonp(result.rows);
		 ////console.log(result.rows);
		//  console.log(result.rows); 
		 var sql = 'select ' + result.rows[0].param_scope +' from alloy_param_data,alloy_shape,test_direction,alloy_param, alloy_standard_info,alloy_report_info,alloy_batch,alloy_test_org where alloy_param_data.param_id=alloy_param.id and  alloy_param_data.standard_id =alloy_standard_info.id and alloy_param_data.report_id =alloy_report_info.id and alloy_param_data.alloy_batch_id =alloy_batch.id and alloy_param_data.alloy_shape_id =alloy_shape.id and alloy_param_data.test_direction_id =test_direction.id and alloy_param_data.test_org_id =alloy_test_org.id and param_id=\'' + req.params.param_id + '\'';
		 console.log(sql);
		 my_conn.query(sql, function (result) {
			 res.jsonp(result.rows);
			 ////console.log(result.rows);
...
router.get('/salt_list50/:param_id', function(req, res,next) { 
 ...
 router.get('/corrode_list50/:alloy_name', function(req, res,next) { 
	...
	//  ['合金名称', '合金批次', '方法依据', '测试温度', '保温时间', '环境气氛', '坩埚材料', '测试装置', '微观腐蚀深度', 'Cr扩散层深度', '失重换算的腐蚀深度', '数据来源', '数据负责人']; 
	 var sql = 'select "alloy_name","alloy_batch","methods","test_temp","duration","environment","crucible","setup","corrdepth_morphology(um)","corrodepth_Cr(um)","corrdepth_weight(um)","references","param_maintainer"  from alloy_salt_corrod left join salt_element on alloy_salt_corrod.salt_batch=salt_element.salt_batch' + ' where alloy_name =\'' + req.params.alloy_name + '\';';
	console.log(sql);
	 my_conn.query(sql,function(result){		
		res.jsonp(result.rows);
		//console.log(result.rows);
