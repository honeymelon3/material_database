 ...
 router.get('/irrad_list50/:param_id', function(req, res,next) { 
	...
	//var sql = 'SELECT column_name from information_schema.columns where table_name = \'alloy_param_data\'' ;
	 ...
	 my_conn.query(sql1, function (result) {
		 ...
		 ////console.log(result.rows);
		 //  console.log(result.rows); 
		 var sql = 'select ' + result.rows[0].param_scope + ' from  alloy_irradiant_param_data,alloy_irradiant_param,alloy_irradiant_report where alloy_irradiant_data.param_id=alloy_irradiant_param.id  and alloy_irradiant_data.report_id =alloy_irradiant_report.id and param_id=\'' + req.params.param_id + '\'';
		 console.log(sql);
		 my_conn.query(sql, function (result) {
			 res.jsonp(result.rows);
			 ////console.log(result.rows);
